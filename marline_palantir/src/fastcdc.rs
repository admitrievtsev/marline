...
impl Iterator for FastCDC<'_> {
    ...
    fn next(&mut self) -> Option<(Chunk, [u32; 12])> {
        if self.remaining == 0 {
            ...
            let (hash, cutpoint, rf) = self.cut(self.processed, self.remaining);
            if cutpoint == 0 {
                None
            } else {
                ...
                Some((Chunk { hash, offset, length }, rf))
            }
        }
    }
...
impl<R: Read> StreamCDC<R> {
    ...
    fn read_chunk(&mut self) -> Result<(ChunkData, [u32; 12]), Error> {
        self.fill_buffer()?;
        if self.length == 0 {
            Err(Error::Empty)
        } else {
            let (hash, count, _) = cut_gear(
